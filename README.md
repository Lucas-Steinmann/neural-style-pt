...
Fork of [ProGamerGov's neural-style-pt](https://github.com/ProGamerGov/neural-style-pt) based on Hydra which allows easier sweeping of hyperparameters.
...
features that are transferred from the style image; you can control this behavior with the `style_scale` parameter.
...
If you set the parameter `original_colors = 1` then the output image will retain the colors of the original image.
...
```
python neural_style.py style_image=examples/inputs/picasso_selfport1907.jpg content_image=examples/inputs/brad_pitt.jpg model_file=models/nin_imagenet.pth gpu=0 backend=cudnn num_iterations=1000 seed=123 content_layers='relu0,relu3,relu7,relu12' style_layers='relu0,relu3,relu7,relu12' content_weight=10 style_weight=500 image_size=512 optimizer=adam
```
...
* `normalize_gradients`: If this parameter is set to `True`, style and content gradients from each layer will be L1 normalized.
...
* `cudnn_autotune`: When using the cuDNN backend, set this parameter to `True` to use the built-in cuDNN autotuner to select
...
**Solution:** Set `cudnn_autotune=True`; this will use the built-in cuDNN autotuner to select the best convolution algorithms.
...
* **Use cuDNN**: Set `backend=cudnn` to use the cuDNN backend. This will only work in GPU mode.
* **Use ADAM**: Set `optimizer=adam` to use ADAM instead of L-BFGS. This should significantly
...
* **Reduce image size**: If the above tricks are not enough, you can reduce the size of the generated image;
  set `image_size=256` to generate an image at half the default size.
...
Here are sometimes for running 500 iterations with `image_size=512` on a Tesla K80 with different settings:
...
* `backend=cudnn cudnn_autotune=True optimizer=lbfgs`: 109 seconds
* `backend=cudnn cudnn_autotune=True optimizer=adam`: 91 seconds
...
* `backend=cudnn cudnn_autotune=True optimizer=lbfgs`: 23 seconds
* `backend=cudnn cudnn_autotune=True optimizer=adam`: 24 seconds

## Multi-GPU scaling
You can use multiple CPU and GPU devices to process images at higher resolutions; different layers of the network will be
computed on different devices. You can control which GPU and CPU devices are used with the `gpu` parameter, and you can control
how to split layers across devices using the `multidevice_strategy` parameter.

For example in a server with four GPUs, you can set `gpu='0,1,2,3'` to process on GPUs 0, 1, 2, and 3 in that order; by also setting `multidevice_strategy='3,6,12'` you indicate that the first two layers should be computed on GPU 0, layers 3 to 5 should be computed on GPU 1, layers 6 to 11 should be computed on GPU 2, and the remaining layers should be computed on GPU 3. You will need to tune the `multidevice_strategy` for your setup in order to achieve maximal resolution.
