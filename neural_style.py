...
def neural_style(params):
    ...
    optimizer, loopVal = setup_optimizer(img, params.optimizer, params.num_iterations, params.lbfgs_num_correction, params.learning_rate)
    while num_calls[0] <= loopVal:
         optimizer.step(feval)


# Configure the optimizer
def setup_optimizer(img, optimizer, num_iterations, lbfgs_num_correction, learning_rate):
    if optimizer == 'lbfgs':
...
def setup_gpu(backend, cudnn_autotune, gpu):
    def setup_cuda():
        if 'cudnn' in backend:
    ...
    elif "c" not in str(gpu).lower():
        setup_cuda()
        dtype, backward_device = torch.cuda.FloatTensor, "cuda:" + str(gpu)
    else:
        setup_cpu()
...
def setup_multi_device(net, gpu, multidevice_strategy):
    ...
    new_net = ModelParallel(net, gpu, multidevice_strategy)
    return new_net
